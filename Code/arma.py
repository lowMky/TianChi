...
def cal_score(original,predicted):
    originalvalue = original.values
    predictedvalue = predicted.values
    diff = originalvalue - predictedvalue
    sum = originalvalue + predictedvalue 
    print originalvalue
    print predictedvalue
    print diff
    print sum
    return (abs(diff/sum)).sum()/len(sum)


def cross_validation(crossdays,p,q,IndexOfShop = None,data = None,DEBUG = False):
...
def predict(predictdays,p,q,IndexOfShop=None,data = None):
    if data == None:
        data = readfile(IndexOfShop) 
    
    #a-all  u-use for predict  p-predict_data
    
    udata,idx = pre_process(data)
    
    ufirstday = datetime.datetime(2015,6,26) + datetime.timedelta(days = idx)
    ulastday = datetime.datetime(2016,10,31)


    pfirstday = ulastday + datetime.timedelta(days = 1)
    plastday = ulastday + datetime.timedelta(days = predictdays)
...
if __name__ == '__main__':
    cross_validation(14,8,1,IndexOfShop = 11,DEBUG = True)
